...
import { getAllBookmarks, type BookmarkNode, moveBookmark, deleteBookmark, createBookmark, subscribeToChanges } from './bookmarks'
import { computeLayout, loadStoredData, saveCanvasState, saveNodePositions, needsFullRelayout, type NodePositions, type CanvasState } from './layout'
import { createCanvasController, type ViewportTransform, transformToCanvas } from './canvas'
import { render, updateNodePosition, highlightFolderCircle, dimOriginCircle, findFolderAtPosition, removeNodeFromDOM, addNodeToDOM } from './render'
...
const canvasController = createCanvasController(canvasContainer, (transform) => {
...
async function init() {
  ...
  await loadAndRender(stored ? !needsFullRelayout(stored) : false, stored?.nodePositions || null)
...
async function loadAndRender(useExistingPositions: boolean, existingPositions: NodePositions | null) {
  const data = await getAllBookmarks()
  allNodes = data.nodes
  allFolders = data.folders

  if (useExistingPositions && existingPositions && Object.keys(existingPositions).length > 0) {
...
let pendingDeletion: { node: BookmarkNode; parentId: string; timeoutId: number } | null = null
...
function handleNodeDragStart(node: BookmarkNode, e: MouseEvent) {
  ...
  const onMouseMove = (moveEvent: MouseEvent) => {
    ...
    const hoveredFolder = findFolderAtPosition(allFolders, positions, canvasPos.x, canvasPos.y)
  ...
  const onMouseUp = async (upEvent: MouseEvent) => {
    ...
    const targetFolder = findFolderAtPosition(allFolders, positions, canvasPos.x, canvasPos.y)
    ...
    if (targetFolder && targetFolder.id !== node.parentId) {
      ...
      const otherBookmarks = allFolders.find((f) => f.title === 'Other Bookmarks')
...
function resetView() {
  if (allNodes.length === 0) {
    canvasController.setTransform({ panX: window.innerWidth / 2, panY: window.innerHeight / 2, zoom: 1 })
    return
  }
